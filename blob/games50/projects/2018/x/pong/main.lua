...
function love.update(dt)
    ...
    elseif gameState == 'play' then
        ...
        if ball.x < 0 then
            ...
            -- if we've reached a score of SCORE_TO_WIN, the game is over; set the
    ...
    if enabledIA then
        if ball.x > (VIRTUAL_WIDTH/3*2) then
            ...
            elseif (ball.y + ball.height/2) > (player2.y + player2.height/3*2) then
                player2.dy = PADDLE_SPEED
            else
                player2.dy = 0
            end
...
function love.draw()
    ...
    love.graphics.setFont(smallFont)
    love.graphics.printf('Player 1', 0, 10, VIRTUAL_WIDTH, 'left')
